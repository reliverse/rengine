#define_import_path degen_toon_terrain::stochastic_sampling 
...
fn get_stochastic_sample_uv(uv: vec2<f32>,   s: f32) -> vec2<f32> {
  ...
  // Smooth interpolation between the two offsets
    let offset = mix(offa, offb, smoothstep(0.2, 0.8, f));

    // Apply the stochastic offset to UV
    return uv + s * offset;
...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    return stochastic_sampling::get_stochastic_sample(in.uv, dpdx(in.uv), dpdy(in.uv), s);
}
