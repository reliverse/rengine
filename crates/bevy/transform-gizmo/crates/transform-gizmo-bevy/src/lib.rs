...
use transform_gizmo::math::{DMat4 as GizmoDMat4, DVec3 as GizmoDVec3, DQuat as GizmoDQuat};
...
fn update_gizmos(
    ...
    *last_cursor_pos = cursor_pos;
    ...
    let Some(viewport): Option<bevy_math::Rect> = camera.logical_viewport_rect() else {
        return;
    };
    ...
    if let Some(custom_viewport) = gizmo_options.viewport_rect {
        let vp_ratio: bevy_math::Vec2 = viewport.size() / custom_viewport.size();
    ...
    let view_matrix: bevy_math::Mat4 = camera_transform.to_matrix().inverse();
    ...
    let gizmo_config = GizmoConfig {
        view_matrix: GizmoDMat4::from_cols(
            view_matrix.as_dmat4().x_axis.into(),
            view_matrix.as_dmat4().y_axis.into(),
            view_matrix.as_dmat4().z_axis.into(),
            view_matrix.as_dmat4().w_axis.into(),
        ),
        projection_matrix: GizmoDMat4::from_cols(
            projection_matrix.as_dmat4().x_axis.into(),
            projection_matrix.as_dmat4().y_axis.into(),
            projection_matrix.as_dmat4().z_axis.into(),
            projection_matrix.as_dmat4().w_axis.into(),
        ),
        viewport,
    ...
    for (entity, mut target_transform, target_global_transform, mut gizmo_target) in &mut q_targets {
        ...
        let gizmo_result = gizmo.update(
            ...
            &[math::Transform {
                ...
                translation: GizmoDVec3::new(
                    target_global_transform.translation.x.into(),
                    target_global_transform.translation.y.into(),
                    target_global_transform.translation.z.into(),
                ).into(),
                rotation: GizmoDQuat::from_xyzw(
                    target_global_transform.rotation.x.into(),
                    target_global_transform.rotation.y.into(),
                    target_global_transform.rotation.z.into(),
                    target_global_transform.rotation.w.into(),
                ).into(),
                scale: GizmoDVec3::new(
                    target_transform.scale.x.into(),
                    target_transform.scale.y.into(),
                    target_transform.scale.z.into(),
                ).into(),
            }],
        );
        ...
        gizmo_target.is_focused = is_focused;
           ...
           if let Some((gizmo_result, updated_targets)) = &gizmo_result {
            ...
            let rotation_delta = ( to_local_rotation * DQuat::from_xyzw(result_transform.rotation.v.x, result_transform.rotation.v.y, result_transform.rotation.v.z, result_transform.rotation.w).as_quat() ) * ( to_local_rotation * target_global_transform.rotation ).inverse();


            target_transform.translation += to_local_rotation * ( DVec3::new(result_transform.translation.x, result_transform.translation.y, result_transform.translation.z).as_vec3() - target_global_transform.translation );
              target_transform.rotation = ( rotation_delta * target_transform.rotation.normalize() ).normalize();



            target_transform.scale = DVec3::new(result_transform.scale.x, result_transform.scale.y, result_transform.scale.z).as_vec3();
        }
        ...
        let gizmo_result = gizmo.update(
            ...
            target_transforms
                ...
                .map(|transform| transform_gizmo::math::Transform {
                   translation: GizmoDVec3::new(
                        transform.1.translation.x.into(),
                        transform.1.translation.y.into(),
                        transform.1.translation.z.into(),
                    ).into(),
                    rotation: GizmoDQuat::from_xyzw(
                        transform.1.rotation.x.into(),
                        transform.1.rotation.y.into(),
                        transform.1.rotation.z.into(),
                        transform.1.rotation.w.into(),
                    ).into(),
                    scale: GizmoDVec3::new(
                        transform.0.scale.x.into(),
                        transform.0.scale.y.into(),
                        transform.0.scale.z.into(),
                    ).into(),
                })
                .collect::<Vec<_>>()
                .as_slice(),
