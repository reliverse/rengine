...
pub(crate) fn tile_edit_plugin(app: &mut App) {
    app
        ...
        .add_systems(Update, update_build_grid_horizontal_offset)
        .add_systems(Update, render_tile_build_grid)
        .add_systems(Update, render_cursor_gizmo)
        .add_systems(Update, listen_for_input_events)
        .add_systems(Update, handle_polygon_tile_build_events)
        .add_systems(Update, handle_rectangle_tile_build_events)
        .add_systems(Update, handle_linear_tile_build_events)
        .add_systems(Update, update_tile_build_preview_rectangle)
        .add_systems(Update, update_tile_build_preview_linear);
}
                ...
                GridInteractionType::Press => {
                    ...
                    if let Ok((builder_entity, mut builder)) = builder_query.single_mut() {
                       ...
                       // if let Some( new_tile_parent_entity ) = new_tile_parent_entity {
                        ...
                        //    commands.entity(block_builder_entity).set_parent_in_place( new_tile_parent_entity );
                        //}
                    }
                }
