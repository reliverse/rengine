//! Copy-pasted from <https://github.com/bevyengine/bevy/blob/main/crates/bevy_math/src/ops.rs>
//!
...
#[expect(
    dead_code,
    reason = "Some math functions happen to be unused. Better to have this ready in case we need them"
)]
mod std_ops {
...
mod libm_ops_for_no_std {
    ...
    /// Precision is specified when the `libm` feature is enabled.
    #[inline(always)]
    pub(crate) fn copysign(x: f32, y: f32) -> f32 {
        libm::copysignf(x, y)
    }
...
#[cfg(feature = "std")]
pub(crate) use std_ops_for_no_std::*;

#[cfg(all(any(feature = "libm", feature = "nostd-libm"), not(feature = "std")))]
pub(crate) use libm_ops_for_no_std::*;
...
#[expect(dead_code, reason = "Better to have this ready in case we need it")]
