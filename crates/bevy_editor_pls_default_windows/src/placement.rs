...
pub fn update_placement_tool_inputs(
  ...
  mut placement_evt_writer: MessageWriter<PlacementEvent>,
  ...
  if key_inputs.pressed(KeyCode::ShiftLeft) {
        ...
        if placement_resource.grid_lock_delay_timer.is_finished() { //only grid lock immediately and then each second 
...
pub fn handle_placement_tool_events(  
  ...
  mut placement_evt_reader: MessageReader<PlacementEvent>,
  mut place_doodad_evt_writer: MessageWriter<PlaceDoodadEvent>, 
