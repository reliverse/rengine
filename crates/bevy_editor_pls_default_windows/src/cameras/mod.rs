...
fn configure_camera_custom(
    ...
    commands.entity(cam_entity).insert(target.clone());
...
fn toggle_editor_cam(
    editor: Res<Editor>,
    mut editor_events: MessageReader<EditorEvent>,
    mut prev_active_cams: ResMut<PreviouslyActiveCameras>,
    mut cam_query: Query<(Entity, &mut Camera, &RenderTarget)>,
) {
    ...
    for event in editor_events.read() {
        if let EditorEvent::Toggle { now_active } = *event {
            if now_active {
                ...
                for (e, mut cam, render_target) in cam_query
                    .iter_mut()
                    //  Ignore non-Window render targets
                    .filter(|(_e, _c, rt)| matches!(rt, RenderTarget::Window(_)))
                    .filter(|(_e, c, _rt)| c.is_active)
                {
