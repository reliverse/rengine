...
impl TxdArchive {
    ...
    fn parse_textures_legacy(buffer: &[u8]) -> Result<Vec<TextureInfo>, RengineError> {
                            ...
                            if is_texture_native {
                                ...
                                if texture_start + 12 <= buffer.len() {
                                    ...
                                    if texture_data_start + struct_size <= buffer.len() {
                                        ...
                                        if let Ok(texture) =
                                            Self::parse_texture_native(texture_data, version, texture_data_start)
                                        {
        ...
        while offset + 12 <= buffer.len() {
            ...
            if let Ok(texture) = Self::parse_texture_native(texture_data, version, offset + 12) { // Pass the absolute offset to texture data start
                textures.push(texture);
            }
