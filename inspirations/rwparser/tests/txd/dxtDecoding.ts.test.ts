...
import { TxdParser, type RwTxd } from "../../src";
...
describe("txd parsing - dxtDecoding", () => {
  ...
  beforeAll(async () => {
    ...
    const txdParser = new TxdParser(
      await readFile(join(__dirname, "../assets/dxtDecoding.txd"))
    );
    rwTxd = txdParser.parse();

    dxtJsOut = JSON.parse(
      await readFile(
        join(__dirname, "../assets/dxtDecoding.dxt-js.json"),
        "utf8"
      )
    );
  ...
  test("texture dictionary - decoding dxt1", () => {
    for (
      let i = 0;
      i < rwTxd.textureDictionary.textureNatives[0].mipmaps[0].length;
      i++
    ) {
      const dxtJsValue = dxtJsOut.DXT1[i];
      const calculatedValue =
        rwTxd.textureDictionary.textureNatives[0].mipmaps[0][i];
  ...
  test("texture dictionary - decoding dxt3", () => {
    for (
      let i = 0;
      i < rwTxd.textureDictionary.textureNatives[1].mipmaps[0].length;
      i++
    ) {
      const dxtJsValue = dxtJsOut.DXT3[i];
      const calculatedValue =
        rwTxd.textureDictionary.textureNatives[1].mipmaps[0][i];
      expect(calculatedValue).toBeLessThanOrEqual(dxtJsValue + 1);
      expect(calculatedValue).toBeGreaterThanOrEqual(dxtJsValue - 1);
    }
  });
  ...
  test("texture dictionary - decoding dxt5", () => {
    for (
      let i = 0;
      i < rwTxd.textureDictionary.textureNatives[2].mipmaps[0].length;
      i++
    ) {
      const dxtJsValue = dxtJsOut.DXT5[i];
      const calculatedValue =
        rwTxd.textureDictionary.textureNatives[2].mipmaps[0][i];
      expect(calculatedValue).toBeLessThanOrEqual(dxtJsValue + 1);
      expect(calculatedValue).toBeGreaterThanOrEqual(dxtJsValue - 1);
    }
  });
