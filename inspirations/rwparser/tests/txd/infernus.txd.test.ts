...
import { TxdParser, type RwTxd } from "../../src/index";
...
describe("txd parsing - infernus", () => {
  ...
  beforeAll(async () => {
    ...
    const txdParser = new TxdParser(
      await readFile(join(__dirname, "../assets/infernus.txd"))
    );
  ...
  test("texture dictionary - length", () => {
    ...
    expect(rwTxd.textureDictionary.textureNatives).toHaveLength(
      rwTxd.textureDictionary.textureCount
    );
  });
  ...
  test("texture dictionary - natives - texture information and mip maps", () => {
    for (const textureNative of rwTxd.textureDictionary.textureNatives) {
      ...
      for (const mipmap of textureNative.mipmaps) {
        ...
        expect(mipmap).toHaveLength(
          textureNative.width * textureNative.height * 4
        );

        // Each color channel is an unsigned byte (0-255)
        const valuesInColorRange = mipmap.every(
    ...
    expect(rwTxd.textureDictionary.textureNatives[0].mipmaps[0][4092]).toBe(
      156
    );
    expect(rwTxd.textureDictionary.textureNatives[0].mipmaps[0][4093]).toBe(
      154
    );
    expect(rwTxd.textureDictionary.textureNatives[0].mipmaps[0][4094]).toBe(
      165
    );
    expect(rwTxd.textureDictionary.textureNatives[0].mipmaps[0][4095]).toBe(
      255
    );
  });

  test("texture dictionary - natives - texture names and mask names", () => {
    expect(rwTxd.textureDictionary.textureNatives[0].textureName).toBe(
      "infernus92wheel32"
    );
    expect(rwTxd.textureDictionary.textureNatives[1].textureName).toBe(
      "infernus92interior128"
    );
    expect(rwTxd.textureDictionary.textureNatives[2].textureName).toBe(
      "infernus92handle32"
    );
