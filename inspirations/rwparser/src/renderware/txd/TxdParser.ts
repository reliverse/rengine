...
export class TxdParser extends RwFile {
  ...
  public readTextureDictionary(): RwTextureDictionary {
    ...
    for (let i = 0; i < textureCount; i++) {
      const textureNative = this.readTextureNative();
  ...
  public readTextureNative(): RwTextureNative {
    ...
    const uAddressing = (flags & 0xf_00) >> 8;
    const vAddressing = (flags & 0xf0_00) >> 12;
        ...
        if (palette.length !== 0) {
          ...
          const hasAlpha =
            ...
            (platformId === PlatformType.D3D8 &&
              !rasterFormatsWithoutAlpha.includes(rasterFormat));

          bitmap = Array.from(
            this.getBitmapWithPalette(
              paletteType,
              depth,
              hasAlpha,
              raster,
          ...
          bitmap = Array.from(
            this.getBitmapWithDXT(
              `DXT${compressionFlags}`,
              raster,
              width,
              height
            )
          ...
          bitmap = Array.from(
            this.getBitmapWithDXT(d3dFormat, raster, width, height)
          );
        } else {
          bitmap = Array.from(
            this.getBitmapWithRasterFormat(rasterFormat, raster, width, height)
          );
        }
  ...
  public readPalette(paletteType: number, depth: number): Uint8Array {
    const size =
      paletteType === PaletteType.PALETTE_8 ? 1024 : depth === 4 ? 64 : 128;

    return this.read(size);
  }

  public getBitmapWithPalette(
    paletteType: number,
    depth: number,
    hasAlpha: boolean,
    raster: Uint8Array,
  ...
  public getBitmapWithRasterFormat(
    rasterFormat: number,
    raster: Uint8Array,
    width: number,
    height: number
