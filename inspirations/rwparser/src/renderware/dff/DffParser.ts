...
export class DffParser extends RwFile {
  ...
  parse(): RwDff {
    ...
    while (this.getPosition() < this.getSize()) {
      ...
      switch (header.sectionType) {
        ...
        default:
          console.debug(
            `Section type ${header.sectionType} (${header.sectionType.toString(16)}) not found at offset (${this.getPosition().toString(16)}). Skipping ${header.sectionSize} bytes.`
          );
      ...
      switch (header.sectionType) {
        ...
        case RwSections.RwExtension: {
          ...
          switch (extensionHeader.sectionType) {
            ...
            default:
              console.debug(
                `Extension type ${extensionHeader.sectionType} (${extensionHeader.sectionType.toString(16)}) not found at offset (${this.getPosition().toString(16)}). Skipping ${extensionHeader.sectionSize} bytes.`
              );
  ...
  public readFrameData(): RwFrame {
    ...
    const coordinatesOffset: RwVector3 = {
      x: this.readFloat(),
      y: this.readFloat(),
      z: this.readFloat(),
    };
  ...
  public readGeometry(versionNumber: number): RwGeometry {
    ...
    if (isTexturedUV1 || isTexturedUV2) {
      for (
        let textureCoordinateIndex = 0;
        textureCoordinateIndex < textureCoordinatesCount;
        textureCoordinateIndex++
      ) {
        textureMappingInformation[textureCoordinateIndex] = [];
        for (let vertexIndex = 0; vertexIndex < vertexCount; vertexIndex++) {
          textureMappingInformation[textureCoordinateIndex][vertexIndex] = {
            u: this.readFloat(),
            v: this.readFloat(),
          };
        }
    ...
    const materialList = this.readMaterialList();
  ...
  public readMaterial(): RwMaterial {
    ...
    if (header.versionNumber > 0x3_04_00) {
  ...
  public readTexture(): RwTexture {
    ...
    const uAddressing = (textureData & 0xf_00) >> 8;
    const vAddressing = (textureData & 0xf0_00) >> 12;
    const usesMipLevels = (textureData & (1 << 16)) !== 0;

    const textureNameSize = this.readSectionHeader().sectionSize;
