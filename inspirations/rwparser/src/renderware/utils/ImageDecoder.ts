...
export class ImageDecoder {
  ...
  static decode565(bits: number): [number, number, number] {
    const r = (bits >> 11) & 0b1_1111;
    const g = (bits >> 5) & 0b11_1111;
    const b = bits & 0b1_1111;

    return [(r << 3) | (r >> 2), (g << 2) | (g >> 4), (b << 3) | (b >> 2)];
  }

  static decode555(bits: number): [number, number, number] {
    const r = Math.round((((bits >> 10) & 0b1_1111) * 255) / 31);
    const g = Math.round((((bits >> 5) & 0b1_1111) * 255) / 31);
    const b = Math.round(((bits & 0b1_1111) * 255) / 31);
    return [r, g, b];
  }
  ...
  static bc1(data: Uint8Array, width: number, height: number): Uint8Array {
    ...
    for (let y = 0; y < height; y += 4) {
      for (let x = 0; x < width; x += 4) {
        ...
        const [c0r, c0g, c0b] = ImageDecoder.decode565(color0);
        const [c1r, c1g, c1b] = ImageDecoder.decode565(color1);
  ...
  static bc3(
    ...
    for (let y = 0; y < height; y += 4) {
      for (let x = 0; x < width; x += 4) {
        ...
        const color0 = ImageDecoder.readUInt16LE(data, offset);
        const color1 = ImageDecoder.readUInt16LE(data, offset + 2);
        const colorBits = ImageDecoder.readUInt32LE(data, offset + 4);
        offset += 8;
