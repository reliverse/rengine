...
Examples:
  ...
  python cli.py ipl-to-godot /path/to/ipl/directory --output /path/to/godot/project --with-rust
  ...
  python cli.py batch extract-imgs /path/to/gta/files --output extracted/
    ...
    def _add_img_parsers(self):
        ...
        # IMG info
        img_info = img_subparsers.add_parser(
            "info", help="Show IMG archive information"
        )
        img_info.add_argument("file", help="IMG file path")
        img_info.add_argument(
        ...
        img_list.add_argument(
            "--format", choices=["table", "json"], default="table", help="Output format"
        )

        # IMG extract
        img_extract = img_subparsers.add_parser(
            "extract", help="Extract files from IMG archive"
        )
        img_extract.add_argument("file", help="IMG file path")
        img_extract.add_argument(
            "files", nargs="*", help="Specific files to extract (empty for all)"
        )
        img_extract.add_argument(
    ...
    def _add_dff_parsers(self):
        ...
        dff_info.add_argument(
            "--detailed", action="store_true", help="Show detailed information"
        )

        # DFF analyze
        dff_analyze = dff_subparsers.add_parser(
            "analyze", help="Analyze DFF model structure"
        )
        dff_analyze.add_argument("file", help="DFF file path")
        dff_analyze.add_argument(
    ...
    def _add_txd_parsers(self):
        ...
        # TXD info
        txd_info = txd_subparsers.add_parser(
            "info", help="Show TXD texture information"
        )
        txd_info.add_argument("file", help="TXD file path")
        txd_info.add_argument(
            "--detailed", action="store_true", help="Show detailed information"
        )

        # TXD extract
        txd_extract = txd_subparsers.add_parser(
            "extract", help="Extract textures from TXD"
        )
        txd_extract.add_argument("file", help="TXD file path")
        txd_extract.add_argument(
            "--output", "-o", required=True, help="Output directory"
        )
        txd_extract.add_argument(
            "--format",
    ...
    def _add_col_parsers(self):
        ...
        # COL info
        col_info = col_subparsers.add_parser(
            "info", help="Show COL collision information"
        )
        col_info.add_argument("file", help="COL file path")
        col_info.add_argument(
            "--detailed", action="store_true", help="Show detailed information"
        )

        # COL analyze
        col_analyze = col_subparsers.add_parser(
            "analyze", help="Analyze COL collision structure"
        )
        col_analyze.add_argument("file", help="COL file path")
        col_analyze.add_argument(
            "--format", choices=["text", "json"], default="text", help="Output format"
        )
    ...
    def _add_ipl_parsers(self):
        ...
        ipl_info.add_argument(
            "--detailed", action="store_true", help="Show detailed information"
        )

        # IPL analyze
        ipl_analyze = ipl_subparsers.add_parser(
            "analyze", help="Analyze IPL map structure"
        )
        ipl_analyze.add_argument("file", help="IPL file path")
        ipl_analyze.add_argument(
            "--format", choices=["text", "json"], default="text", help="Output format"
        )
    ...
    def _add_rw_parsers(self):
        ...
        # RW analyze
        rw_analyze = rw_subparsers.add_parser(
            "analyze", help="Analyze RenderWare file chunks"
        )
        rw_analyze.add_argument("file", help="RenderWare file path (DFF/TXD/COL)")
        rw_analyze.add_argument(
            "--depth", type=int, default=3, help="Maximum chunk depth to analyze"
        )
        rw_analyze.add_argument(
            "--format", choices=["tree", "json"], default="tree", help="Output format"
        )

    def _add_batch_parsers(self):
        """Add batch operation parsers"""
        batch_parser = self.subparsers.add_parser(
            "batch", help="Batch processing operations"
        )
        batch_subparsers = batch_parser.add_subparsers(dest="batch_command")

        # Batch analyze
        batch_analyze = batch_subparsers.add_parser(
            "analyze", help="Analyze multiple files"
        )
        batch_analyze.add_argument("directory", help="Directory to scan")
        batch_analyze.add_argument(
            "--format",
            choices=["dff", "txd", "col", "ipl", "img", "all"],
            default="all",
            help="File format to analyze",
        )
        batch_analyze.add_argument(
            "--recursive",
            "-r",
        ...
        # Batch extract-imgs
        batch_extract = batch_subparsers.add_parser(
            "extract-imgs", help="Extract all IMG archives in directory"
        )
        batch_extract.add_argument("directory", help="Directory containing IMG files")
        batch_extract.add_argument(
            "--output", "-o", required=True, help="Output directory for extracted files"
        )
        batch_extract.add_argument(
            "--recursive",
            "-r",
    ...
    def _add_converter_parsers(self):
        ...
        dff_to_obj = self.subparsers.add_parser(
            "dff-to-obj", help="Convert DFF to OBJ format"
        )
        dff_to_obj.add_argument("file", help="DFF file to convert")
        dff_to_obj.add_argument(
            "--output", "-o", required=True, help="Output OBJ file path"
        )
        dff_to_obj.add_argument("--txd", help="TXD file for materials (optional)")
        dff_to_obj.add_argument(
            "--no-csv", action="store_true",
            help="Disable loading of GTA SA CSV data for model names and TXD paths"
        )

        # TXD to materials converter
        txd_to_materials = self.subparsers.add_parser(
            "txd-to-materials", help="Extract textures and materials from TXD"
        )
        txd_to_materials.add_argument("file", help="TXD file to process")
        txd_to_materials.add_argument(
            "--output",
            "-o",
            required=True,
        ...
        ipl_to_godot = self.subparsers.add_parser(
            "ipl-to-godot", help="Convert IPL map files to Godot project/scene"
        )
        ipl_to_godot.add_argument(
            "input", help="IPL file or directory containing IPL files"
        )
        ipl_to_godot.add_argument(
            "--output",
            "-o",
        ...
        ipl_to_godot.add_argument(
            "--with-rust",
            action="store_true",
            help="Include Rust GDExtension integration and demo scenes (default: False)",
        )
        ipl_to_godot.add_argument(
            "--ide-files",
            nargs="+",
            help="IDE files to use for model name lookup (auto-detected if not specified)",
        )
        ipl_to_godot.add_argument(
            "--convert-assets",
            action="store_true",
            help="Convert DFF models referenced in IPL to OBJ format for Godot",
        )
        ipl_to_godot.add_argument(
            "--dff-path",
            help="Directory containing DFF model files (used with --convert-assets)",
        )
        ipl_to_godot.add_argument(
            "--txd-path",
            help="Directory containing TXD texture files (used with --convert-assets)",
        )
