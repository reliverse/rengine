    ...
    def execute(self, args) -> int:
        try:
            ...
            result = handler.extract_files(
                args.file, args.output, args.files, args.overwrite,
...
class TXDExtractCommand(BaseCommand):
    ...
    def execute(self, args) -> int:
        try:
            ...
            if not getattr(args, 'quiet', False):
                print(f"Extracted {result.get('extracted', 0)} textures to {args.output}")
        ...
        try:
            ...
            if args.format == 'json':
                ...
                for col in analysis['collision_objects']:
                    ...
                    print(f"    Spheres: {col['spheres']}, Boxes: {col['boxes']}, Meshes: {col['meshes']}")
...
class BatchExtractIMGsCommand(BaseCommand):
    ...
    def execute(self, args) -> int:
        img_files = []
        if args.recursive:
            img_files.extend(glob.glob(os.path.join(args.directory, '**', '*.img'), recursive=True))
        else:
        ...
        for img_file in img_files:
            try:
                ...
                # Extract all files
                result = handler.extract_files(
                    img_file, img_output_dir, None, args.overwrite,
                    getattr(args, 'quiet', False)
                )
...
class DFFToOBJCommand(BaseCommand):
    ...
    def execute(self, args) -> int:
        try:
            ...
            result = handler.convert_to_obj(args.file, args.output, args.txd, deps)
...
class TXDToMaterialsCommand(BaseCommand):
    ...
    def execute(self, args) -> int:
        try:
            ...
            result = handler.extract_materials(args.file, args.output, args.format, deps)

            if not getattr(args, 'quiet', False):
                print(f"Extracted {result.get('extracted', 0)} textures from {args.file} to {args.output}")
                if 'message' in result:
                    print(f"Note: {result['message']}")
    ...
    def execute(self, args) -> int:
        try:
            ...
            if os.path.isdir(input_path):
                ...
                if not getattr(args, 'quiet', False):
                    print(f"Converted IPL directory {input_path} to Godot project: {output_path}")
                    print(f"Processed {result.get('ipl_files', 0)} IPL files")
                    print(f"Created {result.get('total_instances', 0)} object instances")
